...
class SVGAVideoEntity {
    ...
    private fun createBitmap(filePath: String): Bitmap? {
        ...
        val cacheBitmap = bitmapsMap[filePath]
    ...
    private fun parserImages(obj: MovieEntity) {
        obj.images?.entries?.forEach { entry ->
            val byteString = entry.value
            if (byteString.size < 4) {
                return@forEach
            }
            if (byteString[0].toInt() == 73 && byteString[1].toInt() == 68 && byteString[2].toInt() == 51) {
                return@forEach
            }
            ...
            if (totalTime.toInt() == 0) {
                ...
                audio.audioKey?.let { imageKey ->
                    ...
                    if (audioCache.exists()) {
                        ...
                        entity.images[imageKey]?.apply {
                            if (size < 4) return@apply
                            val isAudioData =
                                (this[0].toInt() == 73 && this[1].toInt() == 68 && this[2].toInt() == 51) || (this[0].toInt() == -1 && this[1].toInt() == -5 && this[2].toInt() == -108)
